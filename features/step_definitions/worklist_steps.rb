...
Then(/^I can validate Land Charge views totals$/) do
  lcregno = find(:id, 'lc_regn').text
  find(:id,'lc_regn').click
  rws = all('#work-list>tbody').count
  expect(rws).to eq lcregno.to_i 
end

Then(/^I can validate Bankruptcy application views totals$/) do
  bnkreg = find(:xpath, "//*[@id='lc_regn']/span").text
  if bnkreg == true
     find(:xpath, "//*[@id='side-nav']/li[1]/a").click
  end
  rws = all('#work-list>tbody').count
  expect(rws).to eq bnkreg.to_i 
end
